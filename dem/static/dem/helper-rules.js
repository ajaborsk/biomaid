...
function calc_montant(fields, rule, init_call)
 {
  var actions = [];

  var elt_to = document.getElementById(rule.to);
  var to_value = calcul_enveloppe(fields);
  ...
  // 'value':to_value,
  //});
  var elt = document.getElementById(rule.to);
  if (elt) { if (to_value) {elt.innerHTML = to_value;} else {elt.innerHTML = "&nbsp;";} }
  return actions;
 }
...
common_rules = {
  ...
  'obligatoires':{
   ...
   'input_selectors':['#id_nom_projet', '#id_referent', '#id_uf', "#id_libelle", "#id_dyn_uf", "#id_username", "#id_password1", '#id_password2'],
   'func':mandatory,
  },
