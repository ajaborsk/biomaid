        ...
        try:
            ...
            class DemandesADispatcher(AnalyticsEngine):
                ...
                def run(self, verbosity=0):
                    ...
                    self.values['requests'] = Demande.objects.filter(*q_filters, **filters)
                    self.values['count'] = self.values['requests'].count()
            ...
            apps.get_app_config('analytics').register_data_id(
                'dem_demandes_a_repartir',
                {
                    'parameters': {
                        'user': {'default': -1},
                        'dispatcher': {'default': -1},
                        'campaign': {'default': -1},
                        'requester': {'default': -1},
                    },
                    'from': 'DemandesADispatcher(user=user,dispatcher=dispatcher,the_campaign=campaign,requester=requester).requests',
                },
            )
            apps.get_app_config('analytics').register_data_id(
                'dem_nb_demandes_a_repartir',
                {
                    'parameters': {
                        'user': {'default': -1},
                        'dispatcher': {'default': -1},
