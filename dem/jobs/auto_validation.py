...
class Job(MyJob):
    help = _(
        "Valide (soumet) automatiquement les demandes remplissant certains critères "
        "& gère les transferts entre campagnes de recensement"
    )

    def execute(self):
        # executing empty sample job
        logger.info(_("Autovalidation des demandes..."))

        # Pour exécuter une commande (de manage.py):
        from django.core import management

        management.call_command("dem_auto_validation")
        management.call_command("dem_campaign_process")
