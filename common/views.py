...
#     """
#     COPIE TEMPORARAIRE D'UNE METHODE DE CLASSE EQUIVALENTE DANS BiomAidView
...
#             roles.append('ARB')
#         if Calendrier.objects.filter(dispatcher=request.user).exists():
#             roles.append('DIS')
#         roles += list(UserUfRole.objects.filter(user=request.user).values_list('role_code', flat=True).distinct())
#     return set(roles)


# def common_context(request, context={}, **kwargs):
#     """Complète un contexte avec des informations fréquemment utilisées
#
#     Retourne un contexte mis à jour avec :
#     - les titres et les liens des main_menu
...
#
#     # Une campagne n'est accessible que si :
#     # - On est dans la période de recensement (entre les dates de début et de fin)
#     # - On peut saisir des demandes sur au moins une UF d'un établissement qui figure sur
#     #   une des enveloppes/programmes associées à cette campagne/calendrier
#     # TODO: la liste des rôles devrait provenir de la classe Demande (ou d'une classe liée...) et non être incrite en 'dur'...
#
#     # Menu des applications accessibles à l'utilisateur
#     # user_roles = get_user_roles(request)
#     # context['user_roles'] = tuple(user_roles)
#     applications_menu = [{'icon': 'bars', 'label': None, 'entries': []}]
#     for name, value in BiomAidView.biom_aid_applications.items():
#         if value['roles'] & user_roles:
#             applications_menu[0]['entries'].append({'label': value['verbose_name'], 'url_name': name + ':home'})
#     context['applications_menu'] = applications_menu
#
#     context['application_title'] = BiomAidView.biom_aid_applications['dem']['verbose_name']
#
#     context['help_menu'] = (
#         {
#             'icon': 'question',
...
#     )
#
#     context['biom_aid_config'] = apps.app_configs['common'].configs[kwargs.get('config', 'chuap')]
#
#     # Alertes en cours et non lues :
#     # Alertes en cours et non lues :
#     if isinstance(request.user, get_user_model()):
#         current_alerts = Alert.objects.filter(destinataire=request.user, cloture__isnull=True)
#         context['nb_alerts'] = current_alerts.count()
#         context['nb_unread_alerts'] = current_alerts.filter(date_lecture__isnull=True).count()
#         context['user_menu'] = (
#             {
#                 'label': request.user.first_name + " " + request.user.last_name,
#                 'icon': 'user',
...
#             context['user_menu'][0]['icon'] = 'gears'
...
#                 # Le debug_mode est utilisé par les templates
#                 #  et entraîne notamment l'utilisation de bibliothèques js non minifiées
#                 "debug_mode": common.config.settings.DEBUG,
#                 "dem_version": common.config.settings.DEM_VERSION,
#                 "dem_version_date": common.config.settings.DEM_VERSION_DATE,
