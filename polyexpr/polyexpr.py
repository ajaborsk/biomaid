...
class PolyExprTransformer(ast.NodeTransformer):
    def __init__(self, fieldnames, mode='django'):
...
def django_orm_expression(polyexpr: PolyExpr, values: dict, fieldnames: set):
    ...
    tree = ast.fix_missing_locations(PolyExprTransformer([], mode='django').visit(tree))

    # The builtins
    all_vars = {name: value.get('django') for name, value in polyexpr.builtins.items()}
