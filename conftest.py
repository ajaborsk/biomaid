...
#             test_results_dir.joinpath(test.name.replace('/', '-').replace('[', '_').replace(']', '_')).mkdir(
...
#         logging.warn(
#             "The config_filepath was not found, so PyleniumConfig will load with default values."
#             f" File not found: {config_filepath.absolute()}"
#         )
...
#     """
#     test_name = request.node.name.replace('/', '-').replace('[', '_').replace(']', '_')
...
#                     str(test_case.file_path.replace('/', '-').replace('[', '_').replace(']', '_').joinpath("test_failed.png"))
#                 )
...
#     )
#     parser.addoption("--extensions", action="store", default="",
